...
## DocuSign node Requirements:
1. DocuSign Request nodes MUST have an xmi:type attribute using the namespace prefix of ComIbmApplicationConnectorRequest_docusign.msgnode and an attribute applicationConnectorType="docusign"
2. The DocuSign Request node **MUST** be given a schemaPrefix attribute such as schemaPrefix="gen/MessageFlowName.DocuSign_Request" where "MessageFlowName" is the name of the message flow and where "DocuSign_Request" is the type of the message flow node
3. In the ACE Toolkit Application project where the message flow containing the DocuSign Request node is created, you **MUST** create an empty JSON schema file in the gen subdirectory of the Application project where the message flow is created. The JSON schema file should have a name based on the SchemaPrefix mentioned above. The JSON schema file naming should be like MessageFlowName.NodeName.request.schema.json where "MessageFlowName" is the name of the message flow and where "NodeName" is based upon the type of the message flow node.
...
5. The DocuSign Request node **MUST** be given a policyUrl attribute such as "{DiscoveryConnectorPolicyProject}:DocuSign1" where "DiscoveryConnectorPolicyProject" is the name of a policy project in the ACE Toolkit workspace and "DocuSign1" is the name of a .policyxml file in the policy project.
6. For DocuSign Request nodes only the following combinations of displayName, action and businessObject attributes are allowed:
...
1. DocuSign policies MUST be created in a separate Policy Project, NOT in the Application project
...
7. In order to make the Eclipse project an ACE Policy project it **MUST** have a policy.descriptor file in the root of the project directory. This file has a fixed name policy.descriptor. The content of the policy.descriptor looks like this:
   ...
   ```
8. Policy files should be placed in the root of the Policy Project. If the user does not indicate which policy to use you **MUST** create one for the user in the policy project. A good name can be derived from the type of the node followed by a number (starting from 1) such as for example "DocuSign_Request1". Two authentication templates are supported:
